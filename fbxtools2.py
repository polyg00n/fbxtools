        ...
        for filename in files:
            if filename.lower().endswith(".fbx"):
                ...
                if args.rename_find:
                    ...
                    print(f"  > Renaming '{args.rename_find}' to '{args.rename_replace}'")

                # Operation 3: Add Root Bone (only for initial print/info, actual addition happens per-take)
                if add_root:
                    print("  > Root bone injection enabled.")

                # Operation 4: List
                if args.list_skeleton:
                    print_hierarchy(scene.GetRootNode())

                # Read all Take names first so we can process them individually
                take_count = scene.GetSrcObjectCount(fbx.FbxCriteria.ObjectType(fbx.FbxAnimStack.ClassId))
                take_names = []
                for i in range(take_count):
                    take_names.append(scene.GetSrcObject(fbx.FbxCriteria.ObjectType(fbx.FbxAnimStack.ClassId), i).GetName())
                
                # Cleanup the initial scene we used for counting/listing
                scene.Destroy()
                manager.Destroy() # Wipe the manager universe
                
                extracted_clips = []
                if take_names:
                    limit = args.limit if args.limit > 0 else len(take_names)
                    for i in range(min(len(take_names), limit)):
                        target_take_name = take_names[i]
                        
                        # MASTER DESIGNATION
                        is_this_clip_master = False
                        if is_this_master and i == 0:
                            is_this_clip_master = True

                        # Fresh manager per take - guarantees zero memory bleed between clips.
                        take_manager = fbx.FbxManager.Create()
                        take_ios = fbx.FbxIOSettings.Create(take_manager, fbx.IOSROOT)
                        take_manager.SetIOSettings(take_ios)

                        scene = fbx.FbxScene.Create(take_manager, "ExportScene")

                        importer = fbx.FbxImporter.Create(take_manager, "")
                        if not importer.Initialize(file_path, -1, take_ios):
                            print(f"  ! Failed to initialise importer for take {i}, skipping.", flush=True)
                            take_manager.Destroy()
                            continue

                        print(f"  > [take {i}] Initialized importer, starting Import()...", flush=True)

                        # Try the pSubsetIndex overload first. If the Python bindings
                        # don't support it they will raise TypeError, in which case we
                        # fall back to a full import (all takes) and rely on the
                        # SetCurrentAnimationStack + GlobalSettings sync to make Unreal
                        # use the right clip. File sizes will be identical in that case
                        # but the clips will at least play correctly.
                        try:
                            importer.Import(scene, i)
                            print(f"  > [take {i}] Import(scene, i) succeeded (single-take path).", flush=True)
                        except TypeError:
                            print(f"  > [take {i}] pSubsetIndex overload unavailable, falling back to full import.", flush=True)
                            importer.Import(scene)
                        importer.Destroy()
                        print(f"  > [take {i}] Importer destroyed, processing scene...", flush=True)

                        # 1. Apply Normalization
                        if rescale != 1.0:
                            s_root = scene.GetRootNode()
                            for j in range(s_root.GetChildCount()):
                                child = s_root.GetChild(j)
                                cur_scale = child.LclScaling.Get()
                                child.LclScaling.Set(fbx.FbxDouble3(cur_scale[0] * rescale, 
                                                                   cur_scale[1] * rescale, 
                                                                   cur_scale[2] * rescale))
                        if args.rename_find:
                            rename_nodes(scene.GetRootNode(), args.rename_find, args.rename_replace)
                        if add_root:
                            # Re-calling our helper in the new take_manager context
                            add_root_joint(scene, take_manager)

                        # 2. Since we imported only take i, there is exactly one FbxAnimStack
                        # in the scene. Grab it, activate it, and sync the global time span.
                        # Unreal's Interchange importer reads clip bounds from GlobalSettings,
                        # not from the stack itself - without this sync it reports "no data".
                        target_stack = scene.GetSrcObject(
                            fbx.FbxCriteria.ObjectType(fbx.FbxAnimStack.ClassId), 0)

                        if not target_stack:
                            print(f"  ! No anim stack found after import for take {i}, skipping.")
                            take_manager.Destroy()
                            continue

                        scene.SetCurrentAnimationStack(target_stack)
                        take_span = target_stack.GetLocalTimeSpan()
                        scene.GetGlobalSettings().SetTimelineDefaultTimeSpan(take_span)

                        # 3. Final Export
                        clean_take_name = "".join([c for c in target_take_name if c.isalnum() or c in (' ', '_')]).rstrip()
                        output_name = f"{os.path.splitext(filename)[0]}_{clean_take_name}.fbx"
                        output_path = os.path.join(target_depth, output_name)
                        
                        has_root = False
                        r_node = scene.GetRootNode()
                        for j in range(r_node.GetChildCount()):
                            if r_node.GetChild(j).GetName().lower() == "root":
                                has_root = True
                                break
                        root_marker = "ROOT OK" if has_root else "ROOT MISSING"
                        master_marker = " [MASTER]" if is_this_clip_master else ""

                        exporter = fbx.FbxExporter.Create(take_manager, "")
                        if exporter.Initialize(output_path, -1, take_manager.GetIOSettings()):
                            exporter.Export(scene)
                            fsize_mb = os.path.getsize(output_path) / (1024 * 1024)
                            extracted_clips.append((output_name, root_marker))
                            print(f"  > Exported: {output_name}{master_marker} ({root_marker}) [{fsize_mb:.2f} MB]")
                        exporter.Destroy()
                        
                        # Nuclear Destruction of the take manager
                        take_manager.Destroy()
                
                # Logging results for the current file once all clips are extracted
                write_log(output_folder, file_path, extracted_clips, is_this_master)
                
                # Re-create manager for the NEXT file in the loop
                manager = fbx.FbxManager.Create()
                ios = fbx.FbxIOSettings.Create(manager, fbx.IOSROOT)
                manager.SetIOSettings(ios)
            else:
                print("  ! No animation takes found in this FBX file.")
                write_log(output_folder, file_path, [], is_this_master)
